    ...
    void Start ()
    {
        ...
        for (int i = 0; i < height - cellSize; i += cellSize)
        {
            for (int j = 0; j < width - cellSize; j += cellSize)
            {
                ...
                if (obj == Grass)
                {
                    if (Random.Range(-100, 100) % 3 == 0 && (i != cellSize && j != cellSize) && Random.Range(-100, 100) > 0 && count > 0)
                    {
                        obj = IceBox;
                        count--;
                    }                    
        ...
        GameObject.FindGameObjectsWithTag("Ice")[Random.Range(0, GameObject.FindGameObjectsWithTag("Ice").Length)].tag = "Door";
